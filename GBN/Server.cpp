...
int main()
{
	...
	//******************************************************************
	//Staring transmitting process via GBN protocol
	cout << endl << "Press ENTER to start transmitting process via GBN protocol" << endl;
	fgets(req,2,stdin);
	...
	//Sending packets to client whit GBN protocol
	...
	auto t1 = high_resolution_clock::now();					//to get run time of GBN whit W window size
	while(numberOfPacketsSent < totalPackets){
		...
		for(i = 0 ; i < windowSize ; ++i){
			if(acknowledgement.acknowledge[i] == -1 && frame2.packet[i] <= totalPackets){
				if(sentFrameStatus == 0)
					sentFrameStatus = 1;

				numberOfPacketLoss++;
			}
		}
		if(sentFrameStatus == 1)
			cout << "Frame-" << numberOfframesSent << " didn't sent successfully!...There was " << numberOfPacketLoss << " packet loss(s) in sent frame!" << endl;
		else if(sentFrameStatus == 0)
			cout << "Frame-" << numberOfframesSent << " sent successfully!" << endl;
		
		while (sentFrameStatus == 1)
		{
			//Resending frames to client
			cout << endl << "Resending frame-" << numberOfframesSent << " to client." << endl;
			sendto(in, (char*)&frame2, sizeof(frame2), 0, (struct sockaddr *)&clientAddr, clientAddrLen);
			//Wait for aknowledgement from client
			cout  << "Waiting for aknowledgement from client..." << endl;
			recvfrom(in, (char*)&acknowledgement, sizeof(acknowledgement), 0, (struct sockaddr *) &clientAddr, &clientAddrLen);
			sentFrameStatus = 0;
			numberOfPacketLoss = 0;
			for(i = 0 ; i < windowSize ; ++i){
			...
			if(sentFrameStatus == 0)
				cout << "Frame-" << numberOfframesSent << " sent successfully!...Going to send next frame!" << endl;
			else if(sentFrameStatus == 1)
				cout << "Frame-" << numberOfframesSent << " didn't sent successfully!...There was " << numberOfPacketLoss << " packet loss(s) in sent frame!" << endl;
		}
	...
	cout << endl << "*********************************************" << endl;
    cout << endl << "GBN time taken to send " << totalPackets << " packets whit window size of " << windowSize << " in miliseconds as integer is = " << ms_int.count() << "ms" << endl;
    cout << endl << "GBN time taken to send " << totalPackets << " packets whit window size of " << windowSize << " in miliseconds as double is = " << ms_double.count() << "ms" << endl;
	cout << endl << "*********************************************" << endl;
